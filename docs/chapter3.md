...
$$ y(x+h) = y(x) + hy'(x) + \frac{h^2}{2}y''(x) + \cdots \tag{3.3}  $$

というテイラー展開を用いて関数を近似する．現在の値に加えて，一次微分や二次微分などの情報を利用することで，一歩先の値をより高い精度で近似できることを学んだ．
一方，オイラー法では，

$$ y_{n+1} = y_n + hf(x_n, y_n) \tag{3.4} $$

により，現在の点における傾きが次の点まで一定であると仮定し，その傾きを用いて一歩先の値を求める．実装が容易で計算量が少ない反面，刻み幅 h が大きい場合には誤差が大きくなりやすいことを理解した．
修正オイラー法では，

$$ \tilde{y}_{n+1} = y_n + hf(x_n, y_n) \tag{3.5} $$
$$ y_{n+1} = y_n + \frac{h}{2} \left[ f(x_n, y_n) + f(x_n + h, \tilde{y}_{n+1}) \right] \tag{3.6} $$

一度オイラー法によって仮の値を求め，始点と仮の点における傾きを平均することで，オイラー法よりも精度の高い近似解を求める手法であることを学んだ．
さらに，ルンゲ・クッタ法では，

$$ k_1 = f(x_n, y_n)  \tag{3.7} $$
$$ k_2 = f\left(x_n + \frac{h}{2}, y_n + \frac{h}{2} k_1\right) \tag{3.8} $$
$$ k_3 = f\left(x_n + \frac{h}{2}, y_n + \frac{h}{2} k_2\right) \tag{3.9} $$
$$ k_4 = f(x_n + h, y_n + h k_3)  \tag{3.10}$$
$$ y_{n+1} = y_n + \frac{h}{6} (k_1 + 2k_2 + 2k_3 + k_4)  \tag{3.11}$$
